...
function openQV(id) {
  ...
  const images = (Array.isArray(p.imgs) && p.imgs.length) ? p.imgs : [productImage(p)];
...
async function addToCart(p) {
  ...
  if (ex) ex.qty++;
  else cart.push({ id: p.id, name: p.name, price: p.price, cat: p.cat, qty: 1, img: p.imgs[0] });
  saveCart();
  updateCartUI();
  showToast(`${p.name} added to cart`, 'ok');
}
...
function liveSearch(q) {
  ...
  const res = PRODUCTS.filter(p => p.name.toLowerCase().includes(q.toLowerCase()) || p.cat.includes(q.toLowerCase()) || p.desc.toLowerCase().includes(q.toLowerCase()));
...
function renderProductDetail(id) {
  ...
  wrap.innerHTML = `
    <div class="pd-grid">
      <div class="pd-imgs">
        <img class="pd-main-img" id="pdMainImg" src="${p.imgs[0]}" alt="${p.name}" onerror="this.style.opacity=0">
        <div class="pd-thumb-strip">
          ${p.imgs.map((img, i) => `<div class="pd-thumb ${i===0?'active':''}" onclick="switchPDImg(this,'${img}')"><img src="${img}" alt=""></div>`).join('')}
        </div>
      </div>
      <div class="pd-info">
        <div class="pd-bc">
